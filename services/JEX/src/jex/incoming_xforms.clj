...
(defn input-arguments
      ...
      (let [file-metadata (or (:file-metadata condor-map) [])
            arg-prefix    (str "run --rm -a stdout -a stderr -v $(pwd):/de-app-work -w /de-app-work --network host --log-driver=none discoenv/porklock:" (cfg/porklock-tag))]
           (str arg-prefix
...
(defn output-arguments
      ...
      (let [arg-prefix (str "run --rm -a stdout -a stderr -v $(pwd):/de-app-work -w /de-app-work --network host --log-driver=none discoenv/porklock:" (cfg/porklock-tag))]
           (str arg-prefix
                " put --user " user
